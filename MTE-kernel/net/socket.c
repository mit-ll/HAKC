...
#if IS_ENABLED(CONFIG_PAC_MTE_COMPART)
    TRANSFER_STRUCT_TYPE(net) net_transfer;
    TRANSFER_STRUCT_TYPE(socket) socket_transfer;
#endif
...
#if IS_ENABLED(CONFIG_PAC_MTE_COMPART)
    TRANSFER_FUNC_NAME(net)(net, (pf)->create, sizeof(*net), &net_transfer);
    TRANSFER_FUNC_NAME(socket)(sock, (pf)->create, sizeof(*sock),
    &socket_transfer);
		err = ((pf)->create)(HAKC_TRANSFERRED(net, net_transfer),
			 HAKC_TRANSFERRED(socket, socket_transfer), protocol, kern);
		RESTORE_FUNC_NAME(net)(&net_transfer);
		RESTORE_FUNC_NAME(socket)(&socket_transfer);
#else
